...
class GaussianNBAnimation(GaussianNB):
    """
    Creates a 2D GaussianNB decision boundary animation 
    with all chosen parameters this class is inherted from sklearn, 
    so you may chose whatever you like with the init params
    """
    def __init__(self):
        super().__init__() # inheriting from GaussianNB
...
anim = GaussianNBAnimation()
k = anim.animate_training(x_train, y_train, x_test, y_test)
k.save("..\\animations\\gaussianNB.gif", writer='imagemagick')